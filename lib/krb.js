...
function BlockTemplate(template) {
     ...
     */
    this.id = template.workerBlockTempleteID;
...
function getJob(miner, _blockTemplate, bashCache) {
    if (miner.validJobs.size() > 0
        && miner.validJobs.get(0).templateID === _blockTemplate.id
        && !miner.newDiff
        && miner.cachedJob !== null
        && typeof bashCache === 'undefined') {
        return miner.cachedJob;
    }

    let blob = _blockTemplate.nextBlob();
    let target = getTargetHex(miner);
    miner.lastBlockHeight = _blockTemplate.height;

    let newJob = {
        job_id: crypto.pseudoRandomBytes(21).toString('base64'),
        extraNonce: _blockTemplate.workerNonce,
        height: _blockTemplate.height,
        difficulty: miner.difficulty,
        diffHex: miner.diffHex,
        submissions: [],
        templateID: _blockTemplate.id
    };
...
//console.log(miner.id + " get new job: " + newJob.job_id);
    return miner.cachedJob;
}
...
function getMasterJob(pool, workerID) {
    ...
    let workerData = {
        workerBlockTempleteID: crypto.pseudoRandomBytes(21).toString('base64'),
    ...
    let localData = {
        workerBlockTempleteID: workerData.workerBlockTempleteID,
...
function processShare(miner, job, blockTemplate, nonce, resultHash, trust) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        if (trust === true) {
            // console.log("share form trust port, pass");
        }
        else{
